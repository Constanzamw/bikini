...
export default function Corpis() {
  const [publications, setPublications] = useState([]);

  useEffect(() => {
    axios.get(`${baseUrl}/publications`)
      .then(response => {
        const corpis = response.data.flatMap(item => {
          // Filtrar por categoría "BOMBACHAS"
          const filteredPublications = item.publications.filter(publication => publication.cat === "CORPIÑOS");
  ...
  return (
    <div className="bg-gray-100 p-4">
      <Nav />
      <h1 className="text-4xl font-bold mb-4 text-center text-pink-400/40"> CORPIÑOS</h1>
